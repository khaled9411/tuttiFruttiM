...
namespace Gley.DailyRewards.Internal
{
    public class CalendarPopup : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if(transform.GetChild(0).TryGetComponent<Button>(out Button button))
            {
                button.onClick.AddListener(SetupDailyReward.Instance.ShowCharacter);
                button.onClick.AddListener(SetupDailyReward.Instance.HideCalender);
            }
        }
        ...
        private void RefreshText()
        {
            ...
            {
                subtitleText.text = "";
                remainingTime.text = "Your Reward Is Ready";
